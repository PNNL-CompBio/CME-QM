...
def determine_data_type ():
    ...
    # print("Start reading NWchem outfile from stdin! ")
    # print(len(lines)," lines to read")
    for line in lines:
...
def parse_input_tddft (opts):
    ...
    lines = sys.stdin.readlines()
    print("In parse_input_tddft(), reading lines :", len(lines))
    iline = -1
    roots = []
...
if __name__ == "__main__":
    ...
    parser.set_defaults(width=0.1, npoints=2000, units="eV", datafmt="auto", delim="    ", makespec=True, header=True,
                        cchar="#", nbin=20, verbose=False)
